...
class QueriesTable extends Table
{
    ...
    {
        // json encode raw_query
        if (isset($data['raw_query'])) {
            $data['my_query'] = json_encode($data['raw_query'], JSON_THROW_ON_ERROR);
            unset($data['raw_query']);
        }
    }

    public function findVersion1($query, array $options)
    {
        return $query
            ->matching('QueryGroups', function ($q) {
                return $q->where(
                    [
                        'OR' => [
                            ['QueryGroups.version LIKE' => '1.%'],
                            ['QueryGroups.version' => '1']
                        ]
                    ]
                );
            });
